...
public class UserUpdateCallback extends UpdateCallback {

    private boolean isBookStore;
    private Context mContext;

    public UserUpdateCallback(Context mContext, boolean isBookStore) {
    ...
    protected final void hasNewApp(final UpdateAppBean updateAppBean, final UpdateAppManager updateAppManager) {
        ...
        if (AppUpdateUtils.appIsDownloaded(updateAppBean)) {
            ...
            if (DownloadService.isRunning) {//已经存在下载任务
                if (updateAppBean.isConstraint()) {
                    showForceDialog(updateAppBean, AppUpdateUtils.getAppFile(updateAppBean));
                } else {
                ...
                showDialogDownloading(updateAppBean, updateAppManager);
            }
        }
    }
    ...
    protected void show4GDialog(UpdateAppBean updateApp, UpdateAppManager updateAppManager) {
        ...
        updateAppDialog.setLiftButtonListener(dialog -> download(updateAppManager));
     ...
     * @param updateAppManager 网路接口
     */
    protected void showDialogDownloading(UpdateAppBean updateApp, UpdateAppManager updateAppManager) {
        UpdateAppDialog updateAppDialog = new UpdateAppDialog(mContext);
        updateAppDialog.setMessage(updateApp.getUpdateLog());
        updateAppDialog.setLeft(updateApp.isConstraint() ? mContext.getString(R.string.update_app_use_new) : mContext.getString(R.string.update_app_new));
        updateAppDialog.setRight(updateApp.isConstraint() ? mContext.getString(R.string.update_app_quit) : mContext.getString(R.string.update_app_not));
        updateAppDialog.setLiftButtonListener(dialog -> {
        ...
        updateAppDialog.setCancelable(!updateApp.isConstraint());
        updateAppDialog.show();
    }
}
